...
public class AnnotationConfigApplicationContext extends GenericApplicationContext implements AnnotationConfigRegistry {
	 ...
	 * 读取什么呢？还是顾名思义AnnotationBeanDefinition意思是读取一个被加了注解的Bean
	 * 这个类（reader）在构造方法中实例化的
	 */
	private final AnnotatedBeanDefinitionReader reader;
	/**
	 * 同理，这个还是一个扫描器，扫描所有加了注解的Bean
	 * 同样在构造方法中被实例化
	 */
	...
	/**
	 *初始化一个Bean的读取和扫描器
	 * 默认构造函数，如果直接调用这个默认的构造方法，需要稍后通过调用其register()方法
	 * 去注册配置类（javaconfig），并调用refresh方法刷新容器
	...
	public AnnotationConfigApplicationContext() {
		...
		*创建一个读取注解的Bean定义读取器
		 ...
		 *可以用来扫描包或者类，继而转换成bd
		 * 构造方法中扫描类并没有用到
		 * 主要是applicationContext.Scan方法时调用
		 */
